...
function KmlUploadModal({
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const { errors } = await (client.models.flight as any).update({ id: flight.id, kmlS3Key: key });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="bg-darkSurface border border-darkBorder rounded-lg w-full max-w-md mx-4 shadow-2xl overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-darkBorder">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">{flight.date} · {flight.from} → {flight.to}</p>
          </div>
            ...
            <>
              <div
                ...
                className={`border-2 border-dashed rounded-lg p-8 text-center cursor-pointer transition-colors
                  ${file ? "border-gold/50 bg-gold/5" : "border-darkBorder hover:border-gray-500 bg-darkBg"}`}
                onClick={() => document.getElementById("kml-file-input")?.click()}
              >
                <input id="kml-file-input" type="file" accept=".kml" className="hidden" onChange={handleFile} />
                {file ? (
...
function FlightRow({ flight, onKmlClick, onTogglePublish, publishing }: {
  flight: Flight;
  onKmlClick: () => void;
  ...
  return (
    <tr className="border-b border-darkBorder hover:bg-white/[0.02] transition-colors">
      <td className="px-4 py-3 text-xs font-mono text-gray-400 whitespace-nowrap">{flight.date}</td>
      <td className="px-4 py-3">
        <div className="text-sm text-gray-100 font-semibold">{flight.from} → {flight.to}</div>
        {flight.title    && <div className="text-xs text-gray-500 truncate max-w-[180px]">{flight.title}</div>}
        {flight.milestone && <div className="text-xs text-gold">★ {flight.milestone}</div>}
      </td>
      <td className="px-4 py-3 text-xs font-mono text-gray-500 whitespace-nowrap">{flight.aircraftId ?? "—"}</td>
      <td className="px-4 py-3 text-xs text-gray-500 whitespace-nowrap">
        {flight.flightType ? (FLIGHT_TYPE_LABEL[flight.flightType] ?? flight.flightType) : "—"}
      </td>
      <td className="px-4 py-3 text-xs font-mono text-gray-400 whitespace-nowrap">{fmt(flight.totalTime)}</td>
      <td className="px-4 py-3 whitespace-nowrap">
        {flight.kmlS3Key ? (
          <div className="flex items-center gap-2">
            <span className="text-xs text-green-500 font-mono">✓ attached</span>
            <button onClick={onKmlClick} className="text-xs text-gray-600 hover:text-gray-400 underline transition-colors">replace</button>
          </div>
        ) : (
          <button onClick={onKmlClick} className="text-xs px-2.5 py-1 border border-darkBorder rounded text-gray-400 hover:border-gold/50 hover:text-gold transition-colors">
            + attach KML
          </button>
        )}
      </td>
...
export default function AdminFlightsPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        do {
          const { data, nextToken } = await (client.models.flight as any).list({ limit: 1000, nextToken: token });
  ...
  const handleTogglePublish = async (flight: Flight) => {
    ...
    try {
      await (client.models.flight as any).update({ id: flight.id, published: !flight.published });
      setFlights((prev) => prev.map((f) => f.id === flight.id ? { ...f, published: !f.published } : f));
    } finally {
  ...
  const visible = flights.filter((f) => {
    ...
    if (search) {
      ...
      return (
        f.date.includes(q) || f.from.toLowerCase().includes(q) ||
        f.to.toLowerCase().includes(q) || f.aircraftId?.toLowerCase().includes(q) ||
    ...
    <FlyingAdminLayout>
      ...
      <div className="px-6 py-6">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-xl font-bold text-gray-100">Flights</h1>
            <p className="text-xs text-gray-500 font-mono mt-0.5">
              {flights.length} total · {published} published · {noKml} missing KML
            </p>
          </div>
        </div>
          ...
          {(["all", "published", "unpublished", "no-kml"] as const).map((f) => (
            <button
              ...
              className={`text-xs px-3 py-1.5 rounded border transition-colors font-mono
                ${filter === f ? "border-gold/50 text-gold bg-gold/5" : "border-darkBorder text-gray-500 hover:border-gray-500"}`}
            >
        ...
        <div className="border border-darkBorder rounded-lg overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody>
                {loading ? (
                  <tr><td colSpan={7} className="px-4 py-12 text-center text-xs text-gray-600 font-mono animate-pulse">Loading flights…</td></tr>
                ) : visible.length === 0 ? (
                  <tr><td colSpan={7} className="px-4 py-12 text-center text-xs text-gray-600 font-mono">No flights match</td></tr>
                ) : (
                  visible.map((f) => (
                    <FlightRow
                      key={f.id} flight={f}
        ...
        <p className="text-xs text-gray-700 font-mono mt-3">{visible.length} of {flights.length} shown</p>
      </div>
      ...
      {kmlFlight && (
        <KmlUploadModal
          ...
          onSuccess={(key) => {
            setFlights((prev) => prev.map((f) => f.id === kmlFlight.id ? { ...f, kmlS3Key: key } : f));
            setKmlFlight(null);
          }}
        />
      )}
