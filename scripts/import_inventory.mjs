 ...
 * Reads inventory_import.xlsx and seeds the 91 Dispatcher database.
 *
 * Prerequisites:
 *   npm install xlsx @aws-sdk/client-cognito-identity-provider
 *
 * Usage:
 *   node scripts/import_inventory.mjs --user=you@example.com --pass=yourpassword [--dry-run] [--sheet=Ammo]
 */
...
if (!userArg || !passArg) {
  console.error("Usage: node scripts/import_inventory.mjs --user=you@example.com --pass=yourpassword [--dry-run] [--sheet=Ammo]");
  process.exit(1);
}
...
function str(v)  { return v != null && String(v).trim() !== "" ? String(v).trim() : null; }
...
async function createItem(input) {
  if (DRY_RUN) { console.log("  [DRY] item:", input.name, "|", input.category); inserted++; return `dry-${Date.now()}`; }
  try {
    const data = await gql(CREATE_ITEM, { input: { id: randomUUID(), ...input } });
    inserted++;
    return data.createInventoryItem.id;
  } catch (e) { console.error("  ✗ item error:", e.message); errors++; return null; }
}
...
async function importFirearms(wb) {
  ...
  for (const r of readSheet(wb, "Firearms")) {
    ...
    const id = await createItem({
      name, brand: str(r["Brand"]), category: "FIREARM",
      datePurchased: date(r["Date Purchased\n(YYYY-MM-DD)"]) ?? date(r["Date Purchased"]),
      vendor: str(r["Vendor"]), pricePaid: num(r["Total Paid"]),
      currency: currency(r["Currency"]), description: str(r["Description"]), notes: str(r["Notes"]),
    });
    if (!id) continue;
    const input = {
      id: randomUUID(), itemId: id, type: str(r["Type *"]) ?? "OTHER",
      caliber: str(r["Caliber"]), serialNumber: str(r["Serial #"]),
      action: str(r["Action"]), barrelLength: str(r["Barrel Length"]), finish: str(r["Finish"]),
    };
    if (DRY_RUN) { console.log("  [DRY] firearm:", input); await sleep(DELAY_MS); continue; }
    try { await gql(CREATE_FIREARM, { input }); } catch (e) { console.error("  ✗ firearm detail:", e.message); }
...
async function importAmmo(wb) {
  ...
  for (const r of readSheet(wb, "Ammo")) {
    const name = str(r["Name *"]), caliber = str(r["Caliber *"]), qty = int(r["Quantity *"]);
    if (!name || !caliber || qty == null) { if (name) console.warn(`  Skipping "${name}" — missing caliber or quantity`); skipped++; continue; }
    const id = await createItem({
      name, brand: str(r["Brand"]), category: "AMMO",
      datePurchased: date(r["Date Purchased\n(YYYY-MM-DD)"]) ?? date(r["Date Purchased"]),
      vendor: str(r["Vendor"]), pricePaid: num(r["Total Paid"]),
      currency: currency(r["Currency"]), notes: str(r["Notes"]),
    });
    if (!id) continue;
    const rawUnit = str(r["Unit *"])?.toUpperCase();
    const input = {
      id: randomUUID(), itemId: id, caliber, quantity: qty,
      unit: ["ROUNDS","BOX","CASE"].includes(rawUnit) ? rawUnit : "ROUNDS",
      roundsPerUnit: int(r["Rounds / Unit"]), grain: int(r["Grain"]),
      bulletType: str(r["Bullet Type"]), velocityFps: int(r["Velocity (fps)"]),
    };
    if (DRY_RUN) { console.log("  [DRY] ammo:", input); await sleep(DELAY_MS); continue; }
    try { await gql(CREATE_AMMO, { input }); } catch (e) { console.error("  ✗ ammo detail:", e.message); }
    await sleep(DELAY_MS);
  }
}
...
async function importFilaments(wb) {
  ...
  for (const r of readSheet(wb, "Filaments")) {
    ...
    if (!name) { skipped++; continue; }
    const id = await createItem({
      name, brand: str(r["Brand"]), category: "FILAMENT",
      datePurchased: date(r["Date Purchased\n(YYYY-MM-DD)"]) ?? date(r["Date Purchased"]),
      vendor: str(r["Vendor"]), pricePaid: num(r["Total Paid"]),
      currency: currency(r["Currency"]), notes: str(r["Notes"]),
    });
    ...
    const input = {
      id: randomUUID(), itemId: id,
      material: validMats.includes(rawMat) ? rawMat : "OTHER",
      color: str(r["Color"]), weightG: int(r["Weight (g)"]),
      diameter: rawDiam === "2.85" ? "d285" : "d175",
    };
    if (DRY_RUN) { console.log("  [DRY] filament:", input); await sleep(DELAY_MS); continue; }
    try { await gql(CREATE_FILAMENT, { input }); } catch (e) { console.error("  ✗ filament detail:", e.message); }
    await sleep(DELAY_MS);
  }
}
...
async function importInstruments(wb) {
  ...
  for (const r of readSheet(wb, "Instruments")) {
    ...
    if (!name) { skipped++; continue; }
    const id = await createItem({
      name, brand: str(r["Brand"]), category: "INSTRUMENT",
      datePurchased: date(r["Date Purchased\n(YYYY-MM-DD)"]) ?? date(r["Date Purchased"]),
      vendor: str(r["Vendor"]), pricePaid: num(r["Total Paid"]),
      currency: currency(r["Currency"]), description: str(r["Description"]), notes: str(r["Notes"]),
    });
    ...
    const validTypes = ["GUITAR","BASS","AMPLIFIER","PEDAL","KEYBOARD","OTHER"];
    const input = {
      id: randomUUID(), itemId: id,
      type: validTypes.includes(rawType) ? rawType : "OTHER",
      color: str(r["Color"]), strings: int(r["Strings"]),
      tuning: str(r["Tuning"]), bodyMaterial: str(r["Body Material"]), finish: str(r["Finish"]),
    };
    if (DRY_RUN) { console.log("  [DRY] instrument:", input); await sleep(DELAY_MS); continue; }
    try { await gql(CREATE_INSTRUMENT, { input }); } catch (e) { console.error("  ✗ instrument detail:", e.message); }
    await sleep(DELAY_MS);
  }
}
...
async function main() {
  console.log(`91 Dispatcher — Inventory Importer${DRY_RUN ? " (DRY RUN)" : ""}`);
  ...
  const wb = xlsxRead(readFileSync("./inventory_import.xlsx"), { type: "buffer", cellDates: true });

  const run = (name, fn) => !sheetArg || sheetArg === name ? fn(wb) : Promise.resolve();
