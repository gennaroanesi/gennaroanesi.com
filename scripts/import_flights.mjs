 ...
 *
 * Usage:
 *   node import_flights.mjs --user=you@example.com --pass=yourpassword --file=ForeFlight_Export.csv
 *   node import_flights.mjs --user=you@example.com --pass=yourpassword --file=ForeFlight_Export.csv --dry-run
 *
...
if (!userArg || !passArg || !fileArg) {
  console.error("Usage: node import_flights.mjs --user=x --pass=y --file=ForeFlight.csv [--dry-run]");
  process.exit(1);
}
...
function parseApproaches(fields) {
  const approachCols = ["Approach1","Approach2","Approach3","Approach4","Approach5","Approach6"];
  const results = [];
...
function inferMilestone(f) {
  ...
  if (comments.includes("solo") && f.Solo > 0)  return null; // not special enough
  return null;
}

// ── float / int helpers ───────────────────────────────────────────────────────

const f2 = (v) => { const n = parseFloat(v); return isNaN(n) ? null : n; };
const i2 = (v) => { const n = parseInt(v);   return isNaN(n) ? null : n; };
...
for (let i = aircraftStart + 2; i < lines.length; i++) {
  ...
  const rec = {};
  aircraftHeaders.forEach((h, idx) => { rec[h] = vals[idx] ?? ""; });
  if (rec.AircraftID) aircraftMap[rec.AircraftID] = rec.TypeCode || rec.Model || null;
}
...
for (const flight of flights) {
  ...
  if (existingKeys.has(key)) {
    console.log(`  SKIP  ${flight.date}  ${flight.from} → ${flight.to}  (already exists)`);
    skipped++;
    continue;
  }

  if (dryRun) {
    console.log(`  DRY   ${flight.date}  ${flight.from} → ${flight.to}  ${flight.aircraftId ?? ""}  ${flight.totalTime}h  [${flight.flightType}]`);
    created++;
    continue;
  }
  ...
  if (result.errors) {
    console.error(`  ERR   ${flight.date}  ${flight.from} → ${flight.to}`, result.errors[0]?.message);
    errored++;
  } else {
    console.log(`  OK    ${flight.date}  ${flight.from} → ${flight.to}  ${flight.aircraftId ?? ""}  ${flight.totalTime}h  [${flight.flightType}]`);
    created++;
  }
